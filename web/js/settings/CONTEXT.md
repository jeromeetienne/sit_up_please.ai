...
- `settings_dialog.ts`: `SettingsDialog`, which fills its fields from `PomodoroSettings.load()`, writes them back with `PomodoroSettings.save()`, and reports the saved values through its `onSaved` callback.
